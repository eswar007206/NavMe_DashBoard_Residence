...
async function fetchPOIs(): Promise<POI[]> {
  const { data, error } = await supabase
    .from("ar_ropin_pois")
    .select("id, poi_name, poi_type, is_active")
...
export default function BlockShops() {
  ...
  const [status, setStatus] = useState<POIStatus>({});
  const [toasts, setToasts] = useState<Toast[]>([]);

  useEffect(() => {
    if (pois) {
      const init: POIStatus = {};
      pois.forEach((p) => (init[p.id] = p.is_active === true));
      setStatus(init);
    }
  }, [pois]);

  const toggle = useCallback(async (poiId: string, name: string) => {
    const next = !status[poiId];
    setStatus((prev) => ({ ...prev, [poiId]: next }));
    ...
    setToasts((prev) => [...prev, { id: toastId, poiName: name, active: next }]);
    setTimeout(() => {
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
              Enable or disable points of interest
            </p>
          </div>
        </div>
      </motion.div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          >
            {pois?.map((poi) => {
              const isActive = status[poi.id] ?? true;
              return (
                <motion.div
                  key={poi.id}
                  variants={cardItem}
                ...
                >
                  <div className="relative z-10">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <button
                        onClick={() => toggle(poi.id, poi.poi_name)}
          ...
          {toasts.map((toast) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  {toast.active ? "POI is now active" : "POI is now disabled"}
                </p>
              </div>
              <button
                onClick={() =>
