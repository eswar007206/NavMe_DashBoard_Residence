...
import BlueprintFloorPlan from "@/components/BlueprintFloorPlan";
...
export default function Heatmap() {
  ...
  const floorRooms: PlottedRoom[] = useMemo(() => {
    ...
    return rooms
      .filter((r) => {
        // Use floor_no text field (authoritative) — fallback to pos_y threshold
        const floorName = (r.floor_no ?? "").trim().toLowerCase();
        if (floorName === "ground floor") return selectedFloor === "ground";
        if (floorName === "first floor")  return selectedFloor === "first";
        // fallback
        return getFloor(parseNum(r.pos_y)) === selectedFloor;
      })
      .map((r) => ({
        room_id: r.room_id,
        name: r.room_name.trim(),
        x: parseNum(r.pos_x),
        z: parseNum(r.pos_z),
        ...
        {isLoading ? (
          ...
          <div
            ...
            style={{
              ...
              border: "1px solid hsla(var(--glass-border), 0.35)",
            }}
          >
            <BlueprintFloorPlan
              floorRooms={floorRooms}
              userPoints={userPoints}
              onHoverDetail={setSelectedMapDetail}
              isMobile={isMobile}
              floor={selectedFloor}
            />
          </div>
